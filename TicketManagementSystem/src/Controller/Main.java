...
public class Main {
    ...
    private static void executeOperations(Configuration configuration, TicketPool ticketPool) {
    ...
    private static List<Thread> createVendorThreads(Configuration configuration, TicketPool ticketPool) {
    ...
    private static Configuration getConfig(Scanner scanner) {
        while (true) {
            if (new File(CONFIGURATION_FILE).exists()) {
                System.out.print("Previous configuration found. Load it from JSON? (yes/no):");
    ...
    private static Configuration createNewConfig(Scanner scanner) {
        ...
        BigDecimal ticketPrice = getPositiveBigDecimal(scanner);
        int totalTickets = getPositiveInt(scanner, "Enter the total number of tickets: ");
        int ticketReleaseRate = getPositiveInt(scanner, "Enter the ticket release rate (milliseconds): ");
        int customerRetrievalRate = getPositiveInt(scanner, "Enter the customer retrieval rate (milliseconds): ");
        int maxTicketCapacity = getPositiveInt(scanner, "Enter the maximum ticket capacity: ");
    ...
    private static BigDecimal getPositiveBigDecimal(Scanner scanner) {
        BigDecimal value = null;
        while (value == null || value.compareTo(BigDecimal.ZERO) <= 0) {
            try {
                System.out.print("Enter the ticket price (Should be Positive): ");
